...
from seresnet50.model_se_resnext50_bn import BackBonedUnet as Net
ROOT_PATH = 'F:\\WorkStation2\\tgs_pytorch\\'
BRANCH_PATH = 'seresnext50_bn\\amazon\\BackBonedUnet_w_pretrainLovaszComplete'
    ...
    for input, truth, index, cache in valid_loader:
        ...
        with torch.no_grad():
            ...
            if OHEM:
                ...
                loss_origin = net.criterion(logit, truth)
            else:
        ...
        if OHEM:
            valid_loss += batch_size * np.array((loss.item(), dice.item(), 0,loss_origin.item()))
        else:
            valid_loss += batch_size * np.array((loss.item(), dice.item(), 0,loss_origin.item()))
...
def run_train():
    ...
    initial_checkpoint = \
    None
    #'F:/WorkStation2/tgs_pytorch/output/seresnext50_bn/amazon/BestNetComplete/checkpoint/00025000_iou_0.851_model.pth'


    pretrain_file = \
    ROOT_PATH+'\\pretrained\\seresnext50\\se_resnext50_32x4d-a260b3a4.pth'
    ...
    batch_size = 16
    train_dataset = TsgDataset('train_3200_4', train_augment, 'train')
    ...
    valid_dataset = TsgDataset('valid_800_4', valid_augment, 'train')
    ...
    iter_save = [0, num_iters - 1] \
                +list(range(0,num_iters-1,40*iter_per_epoch))
    ...
    if Cos_Annealing==True:
        schduler = lambda x: 0.005+(0.01 / 2) * (np.cos(PI * (np.mod(x - 1, 40 * iter_per_epoch) / (40 * iter_per_epoch))) + 1)
    else:
        schduler = StepScheduler([(0, 0.01),(6000,0.001)])  # None  #LR = StepLR([ (0, 0.01),  (200, 0.001),  (300, -1)])
    ...
    while iter < num_iters:  # loop over the dataset multiple times
        ...
        for input, truth, index, cache in train_loader:
            ...
            if iter % iter_valid == 0 and iter > 0:
                ...
                net.set_mode('train')
                log.write('%0.4f  %5.1f  %6.1f  |  %0.3f  %0.3f  (%0.3f) %0.3f|  %0.3f  %0.3f  |  %0.3f  %0.3f  | %s \n' % ( \
            ...
            if OHEM == True:
                loss = net.focal_loss(logit, truth, 1.0, 0.5, 0.25) + net.criterion(logit, truth)
                # loss = net.focal_loss(logit, truth, 1.0, 0.5, 0.25)
            else:
                loss = net.criterion(logit, truth)
            dice = net.metric(logit, truth)
