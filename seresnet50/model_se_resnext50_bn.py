...
class SeResNeXt50Unet(nn.Module):
    ...
    def __init__(self,dilation=False):
        ...
        self.encoder1 = self.encoder.layer0  # 128
    ...
    def focal_loss(self, output, target, alpha, gamma, OHEM_percent):
        ...
        # Online Hard Example Mining: top x% losses (pixel-wise). Refer to to http://www.robots.ox.ac.uk/~tvg/publications/2017/0026.pdf
...
class SeResNeXt50UnetwoOC(nn.Module):
    def load_pretrain(self, pretrain_file):
        pretrain_dict = torch.load(pretrain_file)
        state_dict = {}
        keys = list(pretrain_dict.keys())
    ...
    def __init__(self,dilation=False):
        ...
        #     nn.ReLU(inplace=True)
        # )
        self.decoder5 = Decoder(2048 + 512, 512, 128)
        self.decoder4 = Decoder(128 + 1024, 256, 128)
        self.decoder3 = Decoder(128 + 512, 128, 64)
        self.decoder2 = Decoder(64 + 256, 64, 64)
        self.decoder1 = Decoder(64, 32, 64)
...
class Unet(nn.Module):
    ...
    def forward(self, x):
        enc1 = self.enc1(x)  # ;print('enc1:',enc1.size())
        enc2 = self.enc2(enc1)  # ;print('enc2:',enc2.size())
        enc3 = self.enc3(enc2)  # ;print('enc3:',enc3.size())
        enc4 = self.enc4(enc3)  # ;print('enc4:',enc4.size())
        center = self.center(enc4)  # ;print('center:',center.size())
        dec4 = self.dec4(
            torch.cat([center, F.upsample(enc4, center.size()[2:], mode='bilinear')],
                      1))  # ;print('dec4:',dec4.size())
        dec3 = self.dec3(
            torch.cat([dec4, F.upsample(enc3, dec4.size()[2:], mode='bilinear')], 1))  # ;print('dec3:',dec3.size())
        dec2 = self.dec2(
            torch.cat([dec3, F.upsample(enc2, dec3.size()[2:], mode='bilinear')], 1))  # ;print('dec2:',dec2.size())
        dec1 = self.dec1(
            torch.cat([dec2, F.upsample(enc1, dec2.size()[2:], mode='bilinear')], 1))  # ;print('dec1:',dec1.size())
        final = self.logit(dec1)
...
class BackBonedUnet(nn.Module):
    ...
    def __init__(self):
        ...
        self.decoder5 = _DecoderBlock(2048 + 512, 512, 256)
        self.decoder4 = _DecoderBlock(256 + 1024, 256, 128)
        self.decoder3 = _DecoderBlock(128 + 512, 128, 64)
        self.decoder2 = _DecoderBlock(64 + 256, 64, 64)
        self.decoder1 = _DecoderBlock(64, 32, 64)

        self.logit = nn.Sequential(
            nn.Conv2d(64 * 9, 64, kernel_size=3, padding=1),
            nn.ReLU(inplace=True),
            nn.Conv2d(64, 1, kernel_size=1, padding=0),
        )
    ...
    def forward(self, x):
        ...
        f = F.upsample(center, scale_factor=2, mode='bilinear', align_corners=True)
        d5 = self.decoder5(torch.cat([f, e5], 1))  # ; print('d5',d5.size())
        d5 = F.upsample(d5, scale_factor=2, mode='bilinear', align_corners=True)
        d4 = self.decoder4(torch.cat([d5, e4], 1))  # ; print('d4',d4.size())
        d4 = F.upsample(d4, scale_factor=2, mode='bilinear', align_corners=True)
...
class BackBonedscSEUnet(nn.Module):
    def load_pretrain(self, pretrain_file):
        pretrain_dict = torch.load(pretrain_file)
        state_dict = {}
        keys = list(pretrain_dict.keys())
    ...
    def __init__(self):
        super(BackBonedscSEUnet, self).__init__()
        self.encoder = se_resnext50_32x4d(1000)
        self.encoder1 = self.encoder.layer0
        self.encoder2 = self.encoder.layer1  # 256
        self.encoder3 = self.encoder.layer2  # 512
        ...
        self.decoder5 = Decoder(2048 + 512, 512, 256)
        self.decoder4 = Decoder(256 + 1024, 256, 128)
        self.decoder3 = Decoder(128 + 512, 128, 64)
        self.decoder2 = Decoder(64 + 256, 64, 64)
        self.decoder1 = Decoder(64, 32, 64)
...
#         )
#
#         self.decoder5 = Decoder(2048 + 512, 512, 256,OC = True)
#         self.decoder4 = Decoder(256 + 1024, 256, 128)
#         self.decoder3 = Decoder(128 + 512, 128, 64)
#         self.decoder2 = Decoder(64 + 256, 64, 64)
#         self.decoder1 = Decoder(64, 32, 64)
